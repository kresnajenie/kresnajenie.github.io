...
const lodLevels = [
  new THREE.SphereGeometry(sphereRadius, 32), // High detail
  new THREE.SphereGeometry(sphereRadius, 16), // Medium detail
  new THREE.SphereGeometry(sphereRadius, 8),  // Lower detail
];
...
const columns = ['X_umap0_norm', 'X_umap1_norm', 'spatial0_norm', 'spatial1_norm','spatial2_norm', 'clusters', 'clusters_colors'];
...
async function updateInstancedMesh(filterType = []) {
  ...
  jsonData.forEach((item, index) => {
      ...
      if (typeof filterType === 'string') {
        ...
        // console.log(filterType)
        if (filterType.includes(item.clusters) || filterType.length == 0) {
          color = new THREE.Color(item.clusters_colors);
      ...
      // dummy1.position.x = item.spatial0_norm *10 * 5;
      // dummy1.position.y = item.spatial1_norm *10 * 5;
      // dummy1.position.z = item.y_spatial2_norm * 1 * 5;
      // dummy1.updateMatrix();
      // instancedMesh.setMatrixAt(index, dummy1.matrix); // Use index * 2 for even indices
      ...
      dummy2.position.x = item.X_umap0_norm * 10 * 10 + offsett;
      dummy2.position.y = item.X_umap1_norm * 10 * 10 ;
      dummy2.position.z = 0 * 10 * 10 ;
      dummy2.updateMatrix();
      instancedMesh.setMatrixAt(index * 2 , dummy2.matrix); // Use index * 2  for odd indices
